<?php
/**
 * Controllers setting Pay grades
...
class PaygradesController extends Zend_Controller_Action {
    ...
    function indexAction() {
    	...
    	$model_report = Front_Model_Report::getInstance();
    	$currencyType = $model_report->getCurrencyType();
    	$this->view->assign('currencyType', $currencyType);
    	
    	$this->_model->primary = $id;
    	$data = $this->_model->getItemById();
    	
    	$model_paygrades->primary = $id;
    	$items = $model_paygrades->getItems();
    	
    	$sub_edit = '';
    	if($id != '' && $sub != '') { 
    		$model_paygrades->currency_id = $sub;
    		$sub_edit = $model_paygrades->getItemById();
    	}
    	
    	$this->view->assign('data', $data);
    	$this->view->assign('items', $items);
    	$this->view->assign('sub_edit', $sub_edit);
    ...
    /**
     * Process pay grades min-max
     */
    function addpaygradesAction() {
    	$row = '';
    	$data	= $this->_getParam('data');
    	$primary= $data['pay_grade_id'];
    	$option = $this->_getParam('option');
    	...
    	$url_back = $this->_base_url.$this->_control."/index/?id=$primary";
    	$this->view->assign('option', $option);
    	$this->view->assign('sub', $sub);
    	
    	$model_paygrades = Front_Model_Paygrades::getInstance();
    	...
    	$validation->required = "Please select Currency";
    	$validation->validNotEmpty('currency_id', $data['currency_id']);
    	...
    	if( !empty($primary) && !empty($sub) && $option == 'edit') { //Update
    		 
    		$model_paygrades->primary = $primary;
    		$model_paygrades->currency_id = $data['currency_id'];
    		$update = $model_paygrades->updateItem($data);
    		...
    		$insert = $model_paygrades->insertItem($data);
    ...
    function listAction(){
    	...
    	$this->view->assign('heading', $this->_lable['job_title']['value']);
    
    	$this->_model->cond = " AND c.avail=1 ";
    
    	$this->_model->_type = 'paygrade';
    	...
    	$general = $preset_general['paygrades'];
    ...
    function delAction() {
    	...
    	$currency_id = $this->_getParam('sub');
    	
    	if($pay_grade_id == '' || $currency_id =='') {
    		$this->_redirect( $this->_base_url.$this->_control."/list/");
    	} 
    	
    	$url_back = $this->_base_url.$this->_control."/index/?id=$pay_grade_id";
    	
    	$model_paygrades = Front_Model_Paygrades::getInstance();
    	
    	$model_paygrades->primary = $pay_grade_id;
    	$model_paygrades->currency_id = $currency_id;
    	$del = $model_paygrades->deleteItem();
